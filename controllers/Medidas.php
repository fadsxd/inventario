...
class Medidas extends Controller{
   ...
   public function listar()
   {
    ...
    for ($i=0; $i < count($data); $i++) { 
        $data[$i]['acciones']='<div>
        <button class="btn btn-danger" type="button" onclick="eliminarMedida('.$data[$i]['id_medida'].')"><i class="fas fa-trash"></i></button>
        <button class="btn btn-info" type="button" onclick="editarMedida('.$data[$i]['id_medida'].')"><i class="fas fa-edit"></i></button>
        </div>';
   ...
   public function registrar(){
    $id_medida = strClean($_POST['id_usuario']);
    $nombre = strClean($_POST['nombre']);
    $nombre_corto = strClean($_POST['nombre_corto']);
    if(empty($nombre)){
        $res = array('msg' => 'EL NOMBRE ES REQUERIDO', 'type' => 'warning');
    }else if(empty($nombre_corto)){
        $res = array('msg' => 'EL SIMBOLO ES REQUERIDO', 'type' => 'warning');
    }else{
        if ($id_medida == '') {
        ...
        if(empty($verificar)){
            ...
            if($data > 0){
                ...
                $res = array('msg' => 'ERROR AL REGISTRADO', 'type' => 'error');
            }
        }else{
            ...
            $verificar = $this->model->getValidar('medida',$nombre,'actualizar',$id_medida);
            if(empty($verificar)){
                $data = $this->model->actualizar($nombre,$nombre_corto,$id_medida);
                if($data == 1){
   ...
   public function listarInactivos()
   {
    ...
    for ($i=0; $i < count($data); $i++) { 
        $data[$i]['acciones']='<div>
        <button class="btn btn-danger" type="button" onclick="restaurarMedida('.$data[$i]['id_medida'].')"><i class="fas fa-check-circle"></i></button>
        </div>';
        }
        echo json_encode($data,JSON_UNESCAPED_UNICODE);
        die();
