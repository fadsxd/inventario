...
class Categorias extends Controller{
    ...
    public function listar(){
        //creamos la funcion getCategorias en el modelo 
        $data = $this->model->getCategorias(1);
        for ($i=0; $i < count($data); $i++) { 
            $data[$i]['acciones'] = ' <div>
            <button class="btn btn-danger" type="button" onclick="eliminarCategoria('.$data[$i]['id_categoria'].')"><i class="fas fa-trash"></i></button>
            <button class="btn btn-info" type="button" onclick="editarCategoria('.$data[$i]['id_categoria'].')"><i class="fas fa-edit"></i></button>
    ...
    public function registrar(){
        if(isset($_POST['nombre'])){
            ...
            if (empty($categoria)) {
                ...
                if ($id == '') {
                    $verificar = $this->model->getValidar('nom_categoria',$categoria,'registrar',0);
                    if (empty($verificar)) {
                    ...
                    $verificar = $this->model->getValidar('nom_categoria',$categoria,'actualizar',$id);
                    if (empty($verificar)) {
    ...
    public function listarInactivos(){
        ...
        for ($i=0; $i < count($data); $i++) { 
            $data[$i]['acciones'] = ' <div>
            <button class="btn btn-danger" type="button" onclick="restaurarCategoria('.$data[$i]['id_categoria'].')"><i class="fas fa-check-circle"></i></button>
            </div>';
        }
